function [y] = nlme_model_enzdegribo(PHI,texp,logconv,nlme_error)
...
if logconv
    t0=exp(PHI(:,4));
    offset = exp(PHI(:,5));
    x0=exp([1; 0; PHI(:,9); PHI(:,7)]);

    [t_out,x] = ode15s(@NLMEtransfection_enzdegribo_ode,[t0,texp(end)], x0, [], exp(PHI));
else
    t0=PHI(:,4);
    offset = PHI(:,5);
    x0=[1; 0; PHI(:,9); PHI(:,7)];

    [t_out,x] = ode15s(@NLMEtransfection_enzdegribo_ode,[t0,texp(end)], x0, [], PHI);
end
